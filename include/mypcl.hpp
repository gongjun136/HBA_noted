...
namespace mypcl
{
   ...
   *
   * @param filePath
   * @param pcd_fill_num pcd文件名中前缀0的数量(例如，如果pcd文件以 00000.pcd开头, 请填写 5)。
   * @param pc
   * @param num
   * @param prefix
   */
  ...
  /**
   * @brief 读取poses.txt文件到pose数组
   *
   * @param filename poses.txt文件路径
   * @param qe 当前寺院数
   * @param te 当前平移
   * @return std::vector<pose> pose数组
   */
  std::vector<pose> read_poses_txt(std::string filename,
                              ...
                              Eigen::Vector3d te = Eigen::Vector3d(0, 0, 0))
  {
    ...
    std::fstream file;
    file.open(filename);
    double time, tx, ty, tz, w, x, y, z;
    while (true) // 无限循环，直到 'break' 语句被执行
    {
        file >> time >> tx >> ty >> tz >> x >> y >> z >> w;
  ...
  /**
   * @brief 读取pose.json文件到pose数组
   *
   * @param filename pose.json文件路径
   * @param qe 当前寺院数
   * @param te 当前平移
   * @return std::vector<pose> pose数组
   */
